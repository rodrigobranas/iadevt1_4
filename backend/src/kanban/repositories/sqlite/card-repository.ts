...
export class SqliteCardRepository implements CardRepository {
  ...
  constructor(private db: Database) {
    ...
    this.deleteStmt = db.prepare(`DELETE FROM cards WHERE id = ?`);
  ...
  async update(cardId: string, input: UpdateCardInput): Promise<Card> {
    ...
    const timestamp = getCurrentTimestamp();
    const labels =
      input.labels !== undefined ? serializeLabels(input.labels) : serializeLabels(card.labels);
    const priority =
      input.priority !== undefined ? validatePriority(input.priority) : card.priority;
