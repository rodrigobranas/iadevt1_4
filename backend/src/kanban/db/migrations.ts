...
export class MigrationRunner {
  ...
  private hasBeenApplied(migrationId: string): boolean {
    const result = this.db.query('SELECT id FROM migrations WHERE id = ?').get(migrationId) as
      | { id: string }
      | undefined;
  ...
  private markAsApplied(migration: Migration): void {
    this.db
      .query('INSERT INTO migrations (id, name, applied_at) VALUES (?, ?, ?)')
  ...
  public async runMigrations(migrations: Migration[]): Promise<void> {
    ...
    for (const migration of migrations) {
      if (this.hasBeenApplied(migration.id)) {
        console.log(
          `⏭️  Migration ${migration.id} (${migration.name}) already applied, skipping...`,
        );
        continue;
      }
